...
#pragma warning restore 1998
        ...
        {
            return  _context.Managers.SingleOrDefault(m => m.ManagerId == id);
        }
        public void DeleteManager(Guid id)
        {
            var manager = _context.Managers.FirstOrDefault(p => p.ManagerId == id);
            if (manager != null)
            {
        ...
        {
            var m = _context.Managers.SingleOrDefault(ma => ma.ManagerId == manager.ManagerId);
            m.Name = manager.Name;
